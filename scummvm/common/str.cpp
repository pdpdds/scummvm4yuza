...
String String::vformat(const char *fmt, va_list args) {
	...
	int len = vsnprintf_(output._str, _builtinCapacity, fmt, va);
	va_end(va);
	...
	if (len == -1 || len == _builtinCapacity - 1) {
		...
		do {
			...
			scumm_va_copy(va, args);
			len = vsnprintf_(output._str, size, fmt, va);
			va_end(va);
		...
		scumm_va_copy(va, args);
		int len2 = vsnprintf_(output._str, len + 1, fmt, va);
		va_end(va);
		assert(len == len2);
		output._size = len2;
	}
